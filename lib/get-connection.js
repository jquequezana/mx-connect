...
const ffi_lib = require('ffi').Library;
const ffi = ffi_lib(null, {
...
const intvlVal = ref.alloc('int', 1);
const intvlValLn = intvlVal.type.size;
    ...
    return new Promise((resolve, reject) => {
        ...
        let tryNextMX = () => {
            ...
            emitConnectHook(err => {
                ...
                // AnyIP freebind support
                let fd = ffi.socket(net.isIPv6(options.localAddress) ? AF_INET6 : AF_INET, SOCK_STREAM, 0);
                let net_socket = new net.Socket({
                    fd
                });
                ffi.setsockopt(net_socket._handle.fd, IPPROTO_IP, IP_FREEBIND, intvlVal, intvlValLn);
                ffi.setsockopt(net_socket._handle.fd, IPPROTO_IP, IP_TRANSPARENT, intvlVal, intvlValLn);

                let socket = net_socket.connect(options, () => {
